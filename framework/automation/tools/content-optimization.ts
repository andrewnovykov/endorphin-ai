 ...
 */
function buildAttributeString(element: Element): string {
  const attrs: string[] = [];
 ...
 */
function buildContentString(element: Element): string {
  const parts: string[] = [];
  ...
  if (text && text.length < 50) parts.push(` text="${text}"`);
  if (value && value.length < 30) parts.push(` value="${value}"`);
  if (href && href.length < 50) parts.push(` href="${href}"`);
  if (element.hasAttribute('disabled')) parts.push(' [disabled]');
  if (className && className.length < 50) parts.push(` class="${className}"`);
 ...
 */
async function createContentChunk(
  page: Page,
  selector: string,
  content: string,
  chunkIndex: number
 ...
 */
async function createPageSnapshot(page: Page, _options: any): Promise<PageSnapshot> {
  const url = page.url();
  ...
  const navContent = await extractSectionContent(
    page,
    'nav, header, .navbar, [role="navigation"], .header'
  );
 ...
 */
function selectOptimalChunks(snapshot: PageSnapshot, instruction?: string): OptimizedContent {
  ...
  if (instruction && isActionInstruction(instruction)) {
    const interactiveChunks = chunks.filter(
      (c) => c.type === 'interactive' && (c.relevanceScore || 0) > 0
    );
...
export function createContentOptimizationTool(framework: EnhancedBrowserTestFramework) {
  return tool(
    async ({ instruction }) => {
      try {
        ...
        const optimizationMetrics = {
          ...
          estimatedSavings:
            snapshot.metadata.estimatedFullPageTokens - optimizedContent.totalTokens,
        };
        ...
        framework.logTestStep(
          ...
          'content-optimization',
          optimizationMetrics,
          `Optimized content: ${optimizationMetrics.selectedChunks}/${optimizationMetrics.totalChunks} chunks, ${optimizationMetrics.tokensUsed} tokens`,
          true
        );
    ...
    },
    {
      ...
      description:
        'Get intelligently optimized page content that dramatically reduces tokens by focusing only on interactive elements and relevant content for the task.',
