...
`;
        ...
        it('should access template files', async () => {
            const templatePath = path.join(testProjectPath, 'node_modules', 'endorphin-ai', 'framework', 'templates', 'reporter', 'report-template.html');
...
`;
        ...
        it('should handle invalid test files gracefully', async () => {
            ...
            fs.writeFileSync(path.join(testProjectPath, 'tests', 'invalid-test.ts'), 'invalid typescript code {');
            const testScript = `
const { TestDiscovery } = require('endorphin-ai/framework/core/test-discovery.js');
...
//# sourceMappingURL=framework-integration.test.js.map