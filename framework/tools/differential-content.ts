...
import { PageSnapshotManager, PageSnapshot, PageDelta } from '../core/page-snapshot.js';

const DifferentialContentSchema = z.object({
  instruction: z.string().describe('The instruction or action being performed'),
  forceFullSnapshot: z.boolean().optional().describe('Force a full snapshot instead of differential'),
});
  ...
  return tool(async ({ instruction, forceFullSnapshot = false }) => {
    try {
      ...
      if (!initialized || forceFullSnapshot) {
        ...
        const fullContent = await generateFullPageContent(page, initialSnapshot);
      ...
      if (!delta) {
        ...
        const fullContent = await generateFullPageContent(page, snapshot);
      ...
      const differentialContent = await generateDifferentialContent(page, delta, snapshotManager);
 ...
 */
async function generateFullPageContent(page: any, snapshot: PageSnapshot): Promise<string> {
 ...
 */
async function generateDifferentialContent(page: any, delta: PageDelta, snapshotManager: PageSnapshotManager): Promise<string> {
  const sections: string[] = [
  ...
  const currentSnapshot = snapshotManager.getCurrentSnapshot();
  if (currentSnapshot && (delta.summary.affectedForms > 0 || delta.summary.affectedInteractive > 0)) {
 ...
 */
async function estimateTokenSavings(page: any, differentialContent: string): Promise<{
