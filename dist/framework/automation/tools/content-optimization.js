...
const contentOptimizationSchema = z.object({
    instruction: z
        .string()
        .optional()
        .describe('The current task instruction to optimize content for'),
});
/**
 ...
 */
async function extractSectionContent(page, selector) {
    try {
        const elements = await page.$$(selector);
        if (elements.length === 0)
            return null;
        const contents = await Promise.all(elements.map((el) => el.evaluate((element) => {
 ...
 */
async function containsInteractiveElements(page, selector) {
    try {
        ...
        for (const interactiveSelector of interactiveSelectors) {
            const count = await page.$$eval(`${selector} ${interactiveSelector}`, (elements) => elements.length);
            if (count > 0)
                return true;
        }
        return false;
 ...
 */
function buildAttributeString(element) {
    const attrs = [];
...
/**
 * Get element description for interactive elements
 */
function getElementDescription(element) {
    const tag = element.tagName.toLowerCase();
    const attributes = buildAttributeString(element);
    const content = buildContentString(element);
    return `${tag}${attributes}${content}`;
}
/**
 * Extract interactive elements with context
 */
async function extractInteractiveElementsContent(page, selector) {
    try {
        const elements = await page.$$(selector);
        if (elements.length === 0)
            return '';
        const elementData = await Promise.all(elements.slice(0, 20).map((el) => {
            // Limit to first 20 elements
            return el.evaluate(getElementDescription);
        }));
...
/**
 * Create content chunk from text
 */
async function createContentChunk(page, selector, content, chunkIndex) {
    return {
 ...
 */
async function splitContentIntoChunks(page, selector, content) {
    const chunks = [];
    const maxChunkSize = 1500;
    if (content.length <= maxChunkSize) {
        const chunk = await createContentChunk(page, selector, content.substring(0, 2000), 0);
        return [chunk];
    }
    ...
    for (const sentence of sentences) {
        ...
        const shouldSplit = combinedLength > maxChunkSize && currentChunk;
        if (shouldSplit) {
            const chunk = await createContentChunk(page, selector, currentChunk, chunkIndex);
            chunks.push(chunk);
 ...
 */
async function createPageSnapshot(page, _options) {
    const url = page.url();
    ...
    for (const selector of contentSelectors) {
        ...
        if (content && content.length > 50) {
            const contentChunks = await splitContentIntoChunks(page, selector, content);
    ...
    const interactiveElements = await page
        .$$eval('button, input, select, textarea, a[href], [onclick], [role="button"]', (elements) => elements.length)
        .catch(() => 0);
 ...
 */
function selectOptimalChunks(snapshot, instruction) {
    ...
    if (instruction && isActionInstruction(instruction)) {
        const interactiveChunks = chunks.filter((c) => c.type === 'interactive' && (c.relevanceScore || 0) > 0);
 ...
 */
function calculateRelevanceScore(chunk, instruction) {
 ...
 */
function isActionInstruction(instruction) {
 ...
 */
function formatChunksForAI(chunks) {
 ...
 */
export function createContentOptimizationTool(framework) {
    return tool(async ({ instruction }) => {
        try {
            ...
            const optimizedContent = selectOptimalChunks(snapshot, instruction);
            // Store snapshot for potential differential analysis
            framework.lastPageSnapshot = snapshot;
            ...
            framework.logTestStep('content-optimization-complete', 'content-optimization', optimizationMetrics, `Optimized content: ${optimizationMetrics.selectedChunks}/${optimizationMetrics.totalChunks} chunks, ${optimizationMetrics.tokensUsed} tokens`, true);
            return {
            ...
            framework.logTestStep('content-optimization-error', 'content-optimization', { error: error.message }, `Content optimization failed: ${error.message}`, false);
...
//# sourceMappingURL=content-optimization.js.map