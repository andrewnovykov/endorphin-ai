...
describe('CustomToolDiscovery - Coverage Enhancement', () => {
  ...
  test('should handle tool creation with various error scenarios', async () => {
    ...
    writeFileSync(join(testTempDir, 'syntax-error-tool.ts'), `
    ...
    writeFileSync(join(testTempDir, 'creation-error-tool.ts'), `
  ...
  test('should handle edge cases in path resolution and validation', async () => {
    ...
    writeFileSync(join(testTempDir, specialCharFile), `
    ...
    writeFileSync(join(nestedDir, 'nested-tool.ts'), `
  ...
  test('should provide comprehensive statistics tracking', async () => {
    ...
    writeFileSync(join(testTempDir, 'valid-tool.ts'), `
    ...
    writeFileSync(join(testTempDir, 'error-tool.ts'), `
      export function createErrorTool(framework) {
        throw new Error('Tool creation error');
      }
    `);

    writeFileSync(join(testTempDir, 'not-a-tool.ts'), `
  ...
  test('should handle getAllTools method', async () => {
    writeFileSync(join(testTempDir, 'test-tool.ts'), `
  ...
  test('should handle files that are not tools', async () => {
    ...
    writeFileSync(join(testTempDir, 'config.ts'), `
      export const config = { setting: 'value' };
    `);

    writeFileSync(join(testTempDir, 'utility.ts'), `
      export function helperFunction() {
        return 'helper';
      }
    `);

    writeFileSync(join(testTempDir, 'readme.md'), `
  ...
  test('should handle mixed file types', async () => {
    ...
    writeFileSync(join(testTempDir, 'ts-tool.ts'), `
    ...
    writeFileSync(join(testTempDir, 'js-tool.js'), `
  ...
  test('should handle directory traversal', async () => {
    ...
    writeFileSync(join(subDir1, 'tool1.ts'), `
    ...
    writeFileSync(join(subDir2, 'tool2.ts'), `
