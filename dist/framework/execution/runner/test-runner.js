...
export class TestRunner {
     ...
     */
    async runTestsInParallel(tests, workers = 2) {
        this.reporter.startSession();
     ...
     */
    async runTestsByCriteria(allTests, criteria, options = {}) {
        ...
        if (criteria.tags && criteria.tags.length > 0) {
            filteredTests = filteredTests.filter((test) => test.tags && criteria.tags.some((tag) => test.tags.includes(tag)));
        }
        // Exclude by tags
        if (criteria.excludeTags && criteria.excludeTags.length > 0) {
            filteredTests = filteredTests.filter((test) => !test.tags || !criteria.excludeTags.some((tag) => test.tags.includes(tag)));
        }
        // Filter by priority
        ...
        if (criteria.pattern) {
            ...
            filteredTests = filteredTests.filter((test) => test.id.toLowerCase().includes(pattern) || test.name.toLowerCase().includes(pattern));
        }
