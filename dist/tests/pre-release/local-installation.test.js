...
describe('Pre-Release Local Installation', () => {
    ...
    describe('Version Validation', () => {
        it('should report correct version', async () => {
            ...
            const sourcePackageJson = JSON.parse(fs.readFileSync(path.join(PROJECT_ROOT, 'package.json'), 'utf8'));
            expect(result.trim()).toBe(sourcePackageJson.version);
        });
        it('should have matching package.json version', async () => {
            const installedPackageJson = JSON.parse(fs.readFileSync(path.join(testProjectPath, 'node_modules', 'endorphin-ai', 'package.json'), 'utf8'));
            const sourcePackageJson = JSON.parse(fs.readFileSync(path.join(PROJECT_ROOT, 'package.json'), 'utf8'));
    ...
    describe('File Integrity', () => {
        ...
        it('should have template files', async () => {
            const templatesPath = path.join(testProjectPath, 'node_modules', 'endorphin-ai', 'framework', 'templates');
...
//# sourceMappingURL=local-installation.test.js.map