...
const log = {
    ...
    debug: (msg, data) => console.debug(`[ContentProcessor] ${msg}`, data || ''),
    warn: (msg, data) => console.warn(`[ContentProcessor] ${msg}`, data || ''),
    error: (msg, error) => console.error(`[ContentProcessor] ${msg}`, error || ''),
};
/**
 * High-performance content processor
 */
export class ContentProcessor {
    tokenEstimationCache = new Map();
     ...
     */
    async optimizePageContent(page, context = {}) {
     ...
     */
    async extractContentChunks(page) {
        const chunks = [];
     ...
     */
    async extractElementChunk(element, type, selector, index) {
        try {
            const data = await element.evaluate((el, _idx) => {
                ...
                if (text.length < 5)
                    return null; // Skip empty/minimal content
                ...
                const hasInteractiveAttributes = el.hasAttribute('onclick') ||
