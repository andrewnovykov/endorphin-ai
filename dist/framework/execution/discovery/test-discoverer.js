...
export class TestDiscoverer {
     ...
     */
    async loadTestFilesConcurrently(testFiles) {
        const results = [];
     ...
     */
    async loadTestFilesConcurrentlyFromMultipleDirs(testFiles) {
        const results = [];
        const maxConcurrency = this.config.maxConcurrency || 5;
        for (let i = 0; i < testFiles.length; i += maxConcurrency) {
            const batch = testFiles.slice(i, i + maxConcurrency);
     ...
     */
    async loadTestFileWithResult(filename) {
        try {
     ...
     */
    async loadTestFileWithResultFromDir(filename, directory) {
        try {
     ...
     */
    async loadTestFileFromDir(filename, directory) {
     ...
     */
    async loadJavaScriptFile(filePath, filename, testsFound) {
     ...
     */
    async loadTypeScriptFile(filePath, filename, testsFound) {
        try {
     ...
     */
    extractTestsFromModule(module, filename, testsFound) {
    ...
    filterTests(criteria) {
        ...
        if (criteria.tags && criteria.tags.length > 0) {
            tests = tests.filter((test) => test.tags && criteria.tags.some((tag) => test.tags.includes(tag)));
        }
        // Exclude by tags
        if (criteria.excludeTags && criteria.excludeTags.length > 0) {
            tests = tests.filter((test) => !test.tags || !criteria.excludeTags.some((tag) => test.tags.includes(tag)));
        }
        // Filter by priority
        ...
        if (criteria.pattern) {
            ...
            tests = tests.filter((test) => test.id.toLowerCase().includes(pattern) || test.name.toLowerCase().includes(pattern));
        }
        return tests;
    }
    /**
    ...
    getStatistics() {
        const tests = this.getAllTests();
        const quarantinedTests = tests.filter((test) => isTestQuarantined(test));
        const priorities = tests.reduce((acc, test) => {
            const priority = test.priority || 'Unknown';
