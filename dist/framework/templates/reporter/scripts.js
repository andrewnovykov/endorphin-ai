...
class TestReportViewer {
        ...
        if (step.screenshots && step.screenshots.length > 0) {
            ...
            if (screenshotsContainer) {
                step.screenshots.forEach((screenshot, idx) => {
                    const imgPath = typeof screenshot === 'object' && screenshot.filepath
                        ? screenshot.filepath
                        : `screenshots/${screenshot}`;
                    const imgName = typeof screenshot === 'object' && screenshot.filename
                        ? screenshot.filename
                        : screenshot;
                    const imgDesc = typeof screenshot === 'object' && screenshot.description
    ...
    filterResults(searchTerm = '', statusFilter = 'all') {
        ...
        rows.forEach((row) => {
            ...
            const searchMatch = !searchTerm || testId.includes(lowerSearchTerm) || testName.includes(lowerSearchTerm);
