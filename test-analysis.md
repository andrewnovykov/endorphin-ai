...
**Root Cause**: The debug manager is not properly categorizing custom tools vs
built-in tools.
...
**Root Cause**: The HTML template doesn't include the expected JavaScript
functions for displaying setup/data results.
...
**Root Cause**: The mock implementation doesn't simulate execution time
properly.
...
**Root Cause**: The test expects the framework to handle errors differently than
throwing them.
