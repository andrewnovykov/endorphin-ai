...
export class HtmlGenerator {
     ...
     */
    processTemplate(template, data, _options) {
        let processedTemplate = template;
        // Calculate token summary from all results
        const totalTokens = data.testResults.reduce((total, result) => {
            return total + (result.tokenSummary?.totalTokens || 0);
        }, 0);
        const totalCost = data.testResults.reduce((total, result) => {
     ...
     */
    generateCustomReport(reportData, templateContent, filename) {
        try {
