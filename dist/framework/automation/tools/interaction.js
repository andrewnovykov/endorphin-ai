        ...
        try {
            ...
            switch (strategy) {
                ...
                case 'exact-text': {
                    ...
                    const buttonCount = await buttonLocator.count();
                    if (buttonCount > 0) {
                        console.log(`🎯 Found ${buttonCount} button(s) with exact text "${selector}", using button strategy`);
                        locator = buttonLocator.first();
                    }
                    else {
                        // Try clickable elements (a, button, input[type=button], etc.)
                        const clickableLocator = framework
                            .currentPage.locator('a, button, input[type="button"], input[type="submit"], [role="button"]')
                            .filter({
                            hasText: new RegExp(`^${selector.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}$`),
                        });
                        const clickableCount = await clickableLocator.count();
                        if (clickableCount > 0) {
                            console.log(`🎯 Found ${clickableCount} clickable element(s) with exact text "${selector}", using first clickable element`);
                            locator = clickableLocator.first();
                        }
                        else {
                            // Fallback to general exact text search (may cause strict mode violation)
                            console.log(`⚠️ Using fallback exact text search for "${selector}" - may cause strict mode violation if multiple elements match`);
                ...
                case 'role': {
                    ...
                    if (name) {
                        ...
                        console.log(`⚠️ Role "${role}" without name may cause strict mode violation - using first match`);
                        locator = framework.currentPage.getByRole(role).first();
                    }
                    break;
                }
                case 'placeholder':
                ...
                default: {
                    ...
                    if (elementCount > 1) {
                        console.log(`⚠️ CSS selector "${selector}" matches ${elementCount} elements - using first element to avoid strict mode violation`);
...
export function createFillTool(framework) {
    return tool(async (params) => {
        ...
        try {
            ...
            framework.logTestStep(stepDesc, 'fill', { selector: finalSelector, value, strategy, clearFirst, pressEnter }, result, success);
 ...
 */
export function createDescribeTool(framework) {
    return tool(async ({ selector, description }) => {
        ...
        try {
            ...
            if (count === 0) {
                const result = `Element ${selector} not found on page`;
                framework.logTestStep(stepDesc, 'describe', { selector, description }, result, false);
                return `❌ ${result}`;
            }
            const result = `Element ${selector} described as "${description}" (found ${count} element${count > 1 ? 's' : ''})`;
            framework.logTestStep(stepDesc, 'describe', { selector, description }, result, true);
            return `✅ ${result}`;
        }
        catch (error) {
 ...
 */
export function createPressSequentiallyTool(framework) {
    return tool(async (params) => {
        ...
        try {
            ...
            if (count === 0) {
                const result = `Element ${selector} not found on page`;
                framework.logTestStep(stepDesc, 'pressSequentially', { selector, text, delay: actualDelay, strategy: actualStrategy }, result, false);
                return `❌ ${result}`;
            }
            ...
            framework.logTestStep(stepDesc, 'pressSequentially', { selector, text, delay: actualDelay, strategy: actualStrategy }, result, true);
            return `✅ ${result}`;
        }
        catch (error) {
            await framework.takeStepScreenshot(`Failed to type sequentially into ${selector}`);
            framework.logTestStep(stepDesc, 'pressSequentially', { selector, text, delay: actualDelay, strategy: actualStrategy }, error.message, false);
