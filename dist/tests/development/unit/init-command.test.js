...
# 5. Run: npx endorphin run test HEALTH-001`;
            ...
            expect(mockFs.writeFile).toHaveBeenCalledWith(path.join(mockTargetDir, '.env'), expect.stringContaining('OPENAI_API_KEY=your_openai_api_key_here'));
            expect(mockFs.writeFile).toHaveBeenCalledWith(path.join(mockTargetDir, '.env'), expect.stringContaining('🎯 How to get your OpenAI API key:'));
        });
...
// - Add your own custom settings here`;
            ...
            expect(mockFs.writeFile).toHaveBeenCalledWith(path.join(mockTargetDir, 'endorphin.config.ts'), expect.stringContaining('// Endorphin AI Configuration'));
            expect(mockFs.writeFile).toHaveBeenCalledWith(path.join(mockTargetDir, 'endorphin.config.ts'), expect.stringContaining('export default {'));
            expect(mockFs.writeFile).toHaveBeenCalledWith(path.join(mockTargetDir, 'endorphin.config.ts'), expect.stringContaining('🎯 Configuration Tips:'));
        });
        it('should handle file copy errors gracefully', async () => {
            ...
            await initProject(mockTargetDir);
            expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringMatching(/⚠️  Examples directory not found at:/));
            expect(consoleWarnSpy).toHaveBeenCalledWith('⚠️  Creating basic configuration files instead...');
        ...
        it('should create all expected files', async () => {
            ...
            expectedFiles.forEach(file => {
                expect(mockFs.writeFile).toHaveBeenCalledWith(path.join(mockTargetDir, file), expect.any(String));
