...
export class AssetManager {
     ...
     */
    cleanupOldReports(maxAge = this.config.maxReportAge) {
        const result = {
            deletedReports: 0,
     ...
     */
    cleanupResults(keepPerTest = this.config.maxReportsPerTest) {
        const result = {
            deletedReports: 0,
            deletedScreenshots: 0,
            freedSpace: 0,
        ...
        try {
            ...
            for (const reportFile of reportFiles) {
                try {
                    ...
                    const screenshotLinks = content.match(/src=["']screenshots\/[^"']+\.(?:png|jpg|jpeg|gif)["']/g);
                    if (screenshotLinks) {
            ...
            if (fs.existsSync(this.config.screenshotsDir)) {
                ...
                result.orphanedScreenshots = allScreenshots.filter((screenshot) => !referencedScreenshots.has(screenshot));
            }
        }
        catch (error) {
