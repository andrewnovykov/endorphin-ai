...
// 🛠️ Simple Enhanced Browser Tools (No MCP Complexity)
...
const getPageContent = tool(async ({ includeTitle = true }) => {
  ...
  const truncatedContent = htmlContent.length > 8000 
  ...
  name: 'getPageContent',
  description: 'Get HTML content of the page with optional title information.',
  schema: z.object({
    includeTitle: z.boolean().optional().describe("Include page title and URL"),
  })
});

const click = tool(async ({ selector, strategy = 'css', timeout = 10000 }) => {
...
const tools = [
  navigate, getPageContent, click, fill, wait, verifyElement, screenshot, getElementInfo
];
