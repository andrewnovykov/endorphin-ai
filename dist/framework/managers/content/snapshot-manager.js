 ...
 */
import { ContentProcessor } from '../../content/processors/content-processor.js';
export class PageSnapshotManager {
    snapshots = new Map();
    currentSnapshotId = null;
    previousSnapshotId = null;
    currentUrl = null;
    maxSnapshots = 3; // Reduced from 10 to save memory
    maxTextLength = 100; // Reduced from 200
    elementCache = new WeakMap(); // Use WeakMap for auto cleanup
    contentOptimizer = new ContentProcessor();
    /**
     * Create optimized content for AI consumption
     */
    async createOptimizedSnapshot(page, context = {}, _id) {
        try {
     ...
     */
    async createFullSnapshot(page, id, options = {}) {
        ...
        try {
            ...
            const interactive = options.includeInteractive !== false ? await this.captureInteractiveSnapshots(page) : [];
            // Capture metadata
     ...
     */
    async compareWithPrevious(page) {
        if (!this.currentSnapshotId) {
            return null;
        }
    ...
    async captureFormSnapshots(page) {
    ...
    async captureInteractiveSnapshots(page) {
        return await page.evaluate(() => {
            function generateSelector(element) {
                if (element.id)
                    return `#${element.id}`;
    ...
    async captureMetadata(page, viewport) {
    ...
    countAffectedForms(changes, forms) {
        const affectedSelectors = new Set(changes.map((c) => c.element.selector));
        return forms.filter((form) => affectedSelectors.has(form.selector) ||
            form.fields.some((field) => affectedSelectors.has(field.selector))).length;
    }
    countAffectedInteractive(changes, interactive) {
        const affectedSelectors = new Set(changes.map((c) => c.element.selector));
