...
export class TestManager {
    framework = null; // Framework instance (if needed)
    testsDir;
    testFiles = [];
    loadedTests = new Map();
    constructor() {
...
export default {
  ...
  task: \`${testData.task}\`
    ...
    getTestsByTag(tag) {
        return Array.from(this.loadedTests.values()).filter((test) => test.tags && test.tags.includes(tag));
    }
    getTestsByPriority(priority) {
        return Array.from(this.loadedTests.values()).filter((test) => test.priority && test.priority === priority);
    }
    getAllTests() {
