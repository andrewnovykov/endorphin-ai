...
import { z } from 'zod';
import type { PageDelta, PageSnapshot } from '../../content/types/snapshot-types.js';
 ...
 */
export function createDifferentialContentTool(framework: EnhancedBrowserTestFramework) {
  // Initialize snapshot manager
  const snapshotManager = new PageSnapshotManager();
  let initialized = false;

  return tool(
    async ({ instruction, forceFullSnapshot = false }) => {
      try {
        ...
        framework.logTestStep(
          'differential-content-start',
          'differential-content',
          { instruction, forceFullSnapshot },
          'Analyzing page changes for AI context',
          true
        );
        ...
        if (!initialized || forceFullSnapshot) {
          ...
          const fullContent = generateFullPageContent(page, initialSnapshot as any);

          framework.logTestStep(
            'differential-content-initial',
            'differential-content',
            {
              elements: (initialSnapshot as any).dom.totalElements,
              forms: (initialSnapshot as any).forms.length,
              interactive: (initialSnapshot as any).interactive.length,
            },
            'Created initial page snapshot',
            true
          );
          ...
          return {
            ...
            content: fullContent,
            metadata: {
              snapshotId: (initialSnapshot as any).id,
              totalElements: (initialSnapshot as any).dom.totalElements,
              interactiveElements: (initialSnapshot as any).interactive.length,
              forms: (initialSnapshot as any).forms.length,
              timestamp: (initialSnapshot as any).timestamp,
            },
          };
        }
        ...
        if (!delta) {
          ...
          const fullContent = generateFullPageContent(page, snapshot as any);
          ...
          return {
            ...
            metadata: {
              ...
              totalElements: (snapshot as any).dom.totalElements,
              interactiveElements: (snapshot as any).interactive.length,
              forms: (snapshot as any).forms.length,
              timestamp: (snapshot as any).timestamp,
        ...
        if (delta.changes.length === 0) {
          ...
          framework.logTestStep(
            'differential-content-no-changes',
            'differential-content',
            { deltaId: delta.snapshotId },
        ...
        framework.logTestStep(
          ...
          `Provided differential content: ${delta.changes.length} changes (${delta.summary.significance} significance)`,
          true
        );
        ...
        framework.logTestStep(
          'differential-content-error',
          'differential-content',
          { error: error.message },
          `Differential content analysis failed: ${error.message}`,
          false
        );
    ...
    },
    {
      ...
      description:
        'Get page content with focus on changes since last interaction. Provides differential updates to minimize token usage while maintaining context.',
 ...
 */
function generateFullPageContent(page: any, snapshot: PageSnapshot): string {
  ...
  if (snapshot.forms.length > 0) {
    ...
    snapshot.forms.forEach((form, formIndex) => {
      ...
      form.fields.forEach((field, fieldIndex) => {
        ...
        sections.push(
          `  ${fieldIndex + 1}. ${field.type}: ${field.selector}${value}${required}${disabled}`
        );
      });
      sections.push('');
    });
 ...
 */
function generateDifferentialContent(
  page: any,
  delta: PageDelta,
  snapshotManager: PageSnapshotManager
): string {
  const sections: string[] = ['Page Changes Detected:', ''];
  ...
  if (
    ...
    if (delta.summary.affectedForms > 0) {
      ...
      currentSnapshot.forms.forEach((form, index) => {
        const affectedFields = form.fields.filter((field) =>
          delta.changes.some((change) => change.element.selector === field.selector)
        );
    ...
    if (delta.summary.affectedInteractive > 0) {
      ...
      currentSnapshot.interactive.forEach((element, index) => {
        if (delta.changes.some((change) => change.element.selector === element.selector)) {
          sections.push(
            `  ${index + 1}. ${element.tagName}: ${element.textContent} (${element.isEnabled ? 'enabled' : 'disabled'})`
          );
        }
      });
    }
 ...
 */
async function estimateTokenSavings(
  page: any,
  differentialContent: string
): Promise<{
  differentialTokens: number;
