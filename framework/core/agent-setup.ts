 ...
 */
export async function setupAgent(tools: any[]): Promise<AgentWorkflow> {
  ...
  function shouldContinue({ messages }: AgentState): string {
    const lastMessage: AgentMessage = messages[messages.length - 1];
  ...
  async function callModel(state: AgentState): Promise<{ messages: any[] }> {
